...
namespace JBooth.MicroSplat
{
   ...
   public class MicroSplatGlobalTexture : FeatureDescriptor
   {
      ...
      public TintLayerCount tintLayerCount = TintLayerCount.One;
      ...
      GUIContent CShaderTintCount = new GUIContent("Layer Count", "Number of global Tint layers");
...
#if __MICROSPLAT_EXTRA_GLOBAL_TINT_LAYERS__
            tintLayerCount = (TintLayerCount)EditorGUILayout.EnumPopup (CShaderTintCount, tintLayerCount);
#endif
            EditorGUI.indentLevel--;
         }
...
#if __MICROSPLAT_EXTRA_GLOBAL_TINT_LAYERS__
               for(int i = 2; i <= (int)tintLayerCount; i++)
               {
                  using (new GUILayout.VerticalScope(GUI.skin.box))
                  {
                     materialEditor.TexturePropertySingleLine(new GUIContent("Tint Texture " + i, "Albedo Tint Texture"), shaderGUI.FindProp("_GlobalTintTex" + i, props));
                     Vector4 parms = mat.GetVector("_GlobalTintBlends");
                     EditorGUI.BeginChangeCheck();
                     parms[i - 2] = EditorGUILayout.Slider("Blend", parms[i - 2], 0, 1);
                     if (EditorGUI.EndChangeCheck())
                     {
                        mat.SetVector("_GlobalTintBlends", parms);
                        EditorUtility.SetDirty(mat);
                     }
                     if (mat.HasProperty("_GlobalTintFade" + i))
                     {
                        Vector4 fade = mat.GetVector("_GlobalTintFade" + i);
                        EditorGUI.BeginChangeCheck();
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                           mat.SetVector("_GlobalTintFade" + i, fade);
                           EditorUtility.SetDirty(mat);
                        }
                     }
                     if (mat.HasProperty("_GlobalTintUVScale" + i))
                     {
                        Vector4 uv = mat.GetVector("_GlobalTintUVScale" + i);
                        Vector2 scale = new Vector2(uv.x, uv.y);
                        Vector2 offset = new Vector2(uv.z, uv.w);
                        ...
                        {
                           uv = new Vector4(scale.x, scale.y, offset.x, offset.y);
                           mat.SetVector("_GlobalTintUVScale" + i, uv);
                           EditorUtility.SetDirty(mat);
                        }
                     }
                  }
      ...
      public override void DrawPerTextureGUI(int index, MicroSplatKeywords keywords, Material mat, MicroSplatPropData propData)
      {
         ...
         if (tintBlendMode != BlendMode.Off)
         {
            ...
            if(perTexTintStr)
            {
               for(int i = 2; i <= (int)tintLayerCount; i++)
               {
                  DrawPerTexFloatSliderNoToggle(index, 30, GetFeatureName(DefineFeature._PERTEXGLOBALTINTSTRENGTH),
                    keywords, propData, (Channel)(i-2), CPerTexTint, 0, 1);
               }
            }
#endif
         }
      ...
      public override void ComputeSampleCounts(string[] features, ref int arraySampleCount, ref int textureSampleCount, ref int maxSamples, ref int tessellationSamples, ref int depTexReadLevel)
      {
         ...
         if (tintBlendMode != BlendMode.Off)
         {
            for(int i = 0; i < (int)tintLayerCount; i++)
            {
               textureSampleCount++;
...
#if __MICROSPLAT_EXTRA_GLOBAL_TINT_LAYERS__
            switch(tintLayerCount)
            {
               case TintLayerCount.Two:
                  features.Add(GetFeatureName(DefineFeature._GLOBALTINT2));
                  break;
               case TintLayerCount.Three:
                  features.Add(GetFeatureName(DefineFeature._GLOBALTINT3));
                  break;
               case TintLayerCount.Four:
                  features.Add(GetFeatureName(DefineFeature._GLOBALTINT4));
                  break;
               case TintLayerCount.Five:
                  features.Add(GetFeatureName(DefineFeature._GLOBALTINT5));
                  break;
            }
#endif
            if (perTexTintStr)
...
#if __MICROSPLAT_EXTRA_GLOBAL_TINT_LAYERS__
            tintLayerCount = HasFeature(keywords, DefineFeature._GLOBALTINT2) ? TintLayerCount.Two : HasFeature(keywords, DefineFeature._GLOBALTINT3) ? TintLayerCount.Three :
                    HasFeature(keywords, DefineFeature._GLOBALTINT4) ? TintLayerCount.Four : HasFeature(keywords, DefineFeature._GLOBALTINT5) ? TintLayerCount.Five : TintLayerCount.One;
#endif
         }
