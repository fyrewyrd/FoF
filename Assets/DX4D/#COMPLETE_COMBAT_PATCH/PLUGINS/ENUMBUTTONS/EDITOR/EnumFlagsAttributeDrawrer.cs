...
public class EnumButtonsAttributeDrawer : PropertyDrawer
{
    ...
    void Initialize(SerializedProperty property)
    {
        ...
        _columnCount = Mathf.Min(_entries.Count, (Mathf.CeilToInt(_entries.Count / 3f) == _rowCount) ? 3 : maxColumns);
    }
    ...
    int[] GetTypedValues(SerializedProperty property, System.Type enumType)
    {
        ...
        if (underlying == typeof(double))
            return ConvertFrom<double>(values);
        else if (underlying == typeof(float))
            return ConvertFrom<float>(values);

        else if (underlying == typeof(string))
            return ConvertFrom<string>(values);

        else if (underlying == typeof(long))
            return ConvertFrom<long>(values);
        else if (underlying == typeof(ulong))
            return ConvertFrom<ulong>(values);
