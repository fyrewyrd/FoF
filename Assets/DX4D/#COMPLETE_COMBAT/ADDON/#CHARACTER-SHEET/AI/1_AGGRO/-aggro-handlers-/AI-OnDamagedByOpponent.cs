...
//{
//    [ServerCallback]
//    public virtual void OnDamagedByOpponent(CharacterSheet attacker, int damageAmount) { }
//}
...
public partial class AI : NetworkBehaviour
{
    ...
    public virtual void OnDamagedByOpponent(CharacterSheet attacker, int damageAmount)
    {
        ...
        //FIX TARGET
        if (!my.target) { my.target = attacker; }
        //TODO: Make the 0.8 a global config var
        else if (attacker != my.target) // don't check distance if same target
        {
            ...
            if (distanceToAttacker < distanceToTarget * 0.8) my.target = attacker;
        }

        //GAIN HATE
        GenerateAggro(my.target, (int)(damageAmount * aggroScale));

        // addon system hooks
        Utils.InvokeMany(typeof(Monster), this, "OnDamagedByEnemy_", attacker, damageAmount);
    }
}
/*
//SPEAK
