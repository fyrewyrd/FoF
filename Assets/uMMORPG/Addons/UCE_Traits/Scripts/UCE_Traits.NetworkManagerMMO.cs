...
partial class NetworkManagerMMO : NetworkManager  // ← NO public/private/protected here
{
    // -----------------------------------------------------------------------------------
    ...
    {
        if (netMsg.traits == null || netMsg.traits.Length == 0) return;

        foreach (int traitId in netMsg.traits)
        {
            if (traitId != 0)
            {
                if (UCE_TraitTemplate.dict.TryGetValue(traitId, out UCE_TraitTemplate trait))
                {
