            ...
            if (NetworkServer.active)
            {
                ...
                if (NetworkTime.time >= nextPartyCheckTime)
                {
                    ...
                    foreach (Player player in playersInInstanceBounds)
                    {
                        ...
                        {
                            int classIndex = NetworkManagerMMO.GetClassIndex(player.className);
                            Transform spawn = ((NetworkManagerMMO)NetworkManager.singleton).GetStartPositionFor(classIndex);
    
                            player.Warp(spawn.position);
    
                            Debug.Log("Removed player " + player.name + " with partyId=" + player.party.partyId 
                                      + " from instance " + name + " with partyId=" + partyId);
                        }
                    }
    ...
    public static Instance CreateInstance(Instance template, int partyId)
    {
        ...
        if (!template.instances.ContainsKey(partyId))
        {
            ...
            if (template.instances.Count < template.instanceLimit)
            {
                ...
                // Use the same visibility range as DistanceInterestManagement on the NetworkManager
                float visRange = 100f;  // ← Change this if you changed it on the NetworkManager
                float zOffset = (template.bounds.size.z + visRange) * partyId;
