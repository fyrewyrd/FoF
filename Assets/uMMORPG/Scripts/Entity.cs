...
#pragma warning restore CS0109 // member does not hide accessible member
    ...
    */public GameObject _target{get{ return my.targeting._target; } set { my.targeting._target = value; } } //[SyncVar] GameObject _target;
    ...
    public bool invincible { get { return character.combat.invincible; } } // GMs, Npcs, ...
    ...
    private bool healthRecovery = true; // can be disabled in combat etc.
    [SerializeField] [HideInInspector] protected LinearInt _healthRecoveryRate = new LinearInt{baseValue=1};
    ...
    private bool manaRecovery = true; // can be disabled in combat etc.
    [SerializeField] [HideInInspector] protected LinearInt _manaRecoveryRate = new LinearInt{baseValue=1};
    ...
    [Header("Damage")]
    [SerializeField] [HideInInspector] protected LinearInt _damage = new LinearInt{baseValue=1};
    ...
    [Header("Defense")]
    [SerializeField] [HideInInspector] protected LinearInt _defense = new LinearInt{baseValue=1};
    ...
    [Header("Skills & Buffs")]*/
    //Skill books edit here
    public ScriptableSkill[] skillTemplates 
    { 
        get 
        { 
            return my.skills.startingSkills.ToArray();
        } 
        set 
        { my.skills.startingSkills = new System.Collections.Generic.List<ScriptableSkill>(value); 
        } 
    }
...
#pragma warning restore CS0649 // Field is never assigned to
    ...
    */
    ...
    // visibility - now handled by DistanceInterestManagement on NetworkManager
    // Visibility is now handled globally by DistanceInterestManagement on the NetworkManager
    // These functions are kept for compatibility with existing code (skills, AI, etc.)

    [Server]
    public void Hide() { }   // no longer does anything - DistanceInterestManagement controls visibility

    [Server]
    public void Show() { }   // no longer does anything

    public bool IsHidden()
    {
        // Always return false. We no longer force-hide individual entities.
        // If you need force hidden behavior later, we can add it back via the AOI system.
        return false;
    }

    public float VisRange()
    {
        // Return the same visibility range that DistanceInterestManagement uses
    ...
    [Server]
    public void OldRevive(float healthPercentage = 1)
    {
    ...
    [Server]
    public virtual void DealOldDamageAt(Entity entity, int amount, float stunChance=0, float stunTime=0)
    {
        int damageDealt = 0;
        ...
        Utils.InvokeMany(typeof(Entity), this, "DealOldDamageAt_", entity, amount, damageDealt, damageType);
    }
    ...
    public bool CastCheckSelf(Skill skill, bool checkSkillReady = true) =>
        CanPayCastingCost(skill.costs) && skill.CheckSelf(this, checkSkillReady);
